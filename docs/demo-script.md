...
| 1:50–3:00 | Phone: request a large amount → laptop **dashboard lights up "needs human"** → operator opens review → **approves on the physical Ledger** | This one's too big for the agent's mandate, so it escalates. The operator sees the agent's reasoning, the customer's history, can chat with them — then approves on hardware. Only now does Arc disburse. |
| 3:00–3:30 | Phone: same person requests again → **blocked "repay first"** → operator clicks **Mark repaid** → freed | One open tab at a time — no stacking. Repayment frees the credit and raises reputation. |
...
operator approves on the Ledger → "and the same person can't request more credit until they repay." Done.

## The line to repeat

> "One verified human, one credit line. The agent handles the everyday tabs on
